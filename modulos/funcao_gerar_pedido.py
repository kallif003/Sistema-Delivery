def gerando_pedido(modulo, cursor, banco10, data, time, valor, tela1, tela2, tela3):
    try:
        ...
        sql = "insert into gerenciarPedido(telefone, nome, cep, endereco, numero, bairro, referencia, complemento, taxaEntrega, dataa, hora, valorTotal) values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        insert = (
            str(dados[0][1]), str(dados[0][2]), str(dados[0][3]), str(dados[0][4]), str(dados[0][5]), str(dados[0][6]),
            str(dados[0][7]), str(dados[0][8]), str(dados[0][9]), data, str(hora), valorTotal)
        ...
        sql2 = "insert into status_pedido(st_pedido, hora, motoboy, hora_saida, hora_chegada, id_pedido) values (%s, %s, %s, %s, %s, %s)"
        insert2 = (str('Em preparo'), str(hora), str(''), str(''), str(''), str(id_pedido[0][0]))
        cursor.execute(sql2, insert2)
        banco10.commit()

        gerar_pedido.pedido(cursor, banco10, id_pedido, data, data2, desc, din, troco, cartao, voucher, pix, obs, valorTotal, dados, hora)
